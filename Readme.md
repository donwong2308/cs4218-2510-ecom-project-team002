...
- Zoebelle:
  ...
  - **Milestone 2 – Integration Testing (Frontend):**
      - useCategory.integration.test.js – Hook API integration and async state management  
      - layout.integration.test.js – Header/Footer composition, Helmet metadata  
      - spinner.integration.test.js – Countdown redirect logic, accessibility, and cleanup
      - Header.integration.test.js – Navbar routes, auth states, logout flow, responsive toggler.
      - Footer.integration.test.js – Structure, links (`/about`, `/contact`, `/policy`), and accessibility.  
      - Layout.integration.test.js – Composed frame (Header + Footer + Helmet metadata).  
      - context/search.integration.test.js` – State sync across providers and components.  
      - Categories.integration.test.js – Category list rendering, empty state, and slug links.
      - About.integration.test.js – Image/text layout, Helmet metadata, and navigation.  
      - Pagenotfound.integration.test.js – 404 page rendering, “Go Back” link, and layout frame.  
      - pages/Search.integration.test.js – Search results display, empty-state handling, and layout consistency.  
      - components/Form/SearchInput.js – Search field behavior (prevent empty call, context update, `/search` navigation).  
